...
layout: ../../layout.astro
title: React Native
description: React Native bindings for @chimba-ui/state-machine.
---
...
Re-exported directly from `@chimba-ui/react-state-machine` — identical on both platforms because React Native uses the same React renderer. See the [React](/libs/react) page for full docs on these.

```tsx
import { useMachine, useSelector, type ComponentEffect } from '@chimba-ui/native-state-machine'
```
...
```tsx
import { mergeProps, normalize } from '@chimba-ui/native-state-machine'
...
import type { ComponentEffect } from '@chimba-ui/native-state-machine'
...
```tsx
// web — Disclosure.tsx
import { useMachine, normalize } from '@chimba-ui/react-state-machine'
...
import { useMachine, normalize } from '@chimba-ui/native-state-machine'
import { createDisclosureConfig, connectDisclosure } from '../shared/disclosure'
