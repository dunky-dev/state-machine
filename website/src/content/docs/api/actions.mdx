...
`act()` is shorthand for a context-writing action. It drops the `$ => $.setContext(...)` wrapper, and takes one or many patches (applied in order; a later patch function sees earlier writes):
