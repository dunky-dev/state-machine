...
combobox.stop() // stops all members and disposes sync/combine

popup.send({ type: 'focus' })
submenu.send({ type: 'open' })
```

## `sync` for cross-region coordination

React when any member changes. It's coarse: it wakes on any change to any member, and you read what you need inside:
